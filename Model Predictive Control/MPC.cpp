...
//const size_t deltaL_start = epsi_start + N;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int k = 0; k < N - 2; k++) {
      //fg[0] += 20*CppAD::pow(vars[deltaL_start + k + 1] - vars[deltaL_start + k], 2);
      fg[0] += 200000*CppAD::pow(vars[delta_start + k + 1] - vars[delta_start + k], 2);
	...
	//fg[1 + deltaL_start] = vars[deltaL_start];
    ...
    for (int t = 1; t < N; t++) {
	  ...
	  //AD<double> deltaL1 = vars[deltaL_start + t];
	  ...
	  //AD<double> deltaL0 = vars[deltaL_start + t - 1];
	  ...
	  //fg[1 + deltaL_start + t] = deltaL1 - (0.*deltaL0 + 1.0*delta0);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  const size_t n_vars = N * n_states + (N - 1) * 2;
  // TODO: Set the number of constraints
  const size_t n_constraints = N * n_states;
  ...
  //constraints_lowerbound[deltaL_start] = delta;
  ...
  //constraints_upperbound[deltaL_start] = delta;
